...
def genData(dataStr,count):
    ...
    for i in re.finditer(r"(autodate\('(\d{4}\-\d{1,2}\-\d{1,2}( \d{2}:\d{2}:\d{2})?)',(['\-',\d]+)(,datetype='(\w+)')?(,informat='(['%','Y','m','d','H','M','S','/',' ','\-','\:']+)')?(,outformat='(['%','Y','m','d','H','M','S','/',' ','\-','\:']+)')?(,flag='(\w+)')?\))",dataStr):
...
def mockit(sourcefile,filename,**kwargs):
    mockfile = os.path.join(os.path.split(os.path.realpath(sourcefile))[0], "files", filename)
    tmpfile = os.path.join(os.path.split(os.path.realpath(sourcefile))[0], "files", "tmp.json")
    file = unicode(mockfile , "utf8")
    content = None
    with open(file, "r") as f:
        content = f.read()
    if kwargs:
        for k,v in kwargs.items():
            for i in range(content.count("{%s}" %k)):
                content = content.replace("{%s}" %k,v)
    with open(tmpfile, "w") as f:
        f.write(content)

    data = {"config":open(tmpfile,"rb")}
    r = requests.post(config.mockurl,files=data)
    if r.status_code != 200 or r.json()["code"] != "200" or r.json()["message"] != "配置成功":
        print "Warning:"+r.text
    else:
        print "[Mock]success:%s" %filename
...
if __name__ == "__main__":
    ...
    #                 "call_time":"autodate('2016-12-13',0,datetype='day',flag='calltime')",
